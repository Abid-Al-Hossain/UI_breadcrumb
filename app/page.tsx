...
import { DEFAULT_BREADCRUMB_STATE } from "./_data/BreadcrumbPresets";
...
import { SECTIONS, type SectionId, type BreadcrumbState, type StudioPreset } from "./types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<BreadcrumbState>(DEFAULT_BREADCRUMB_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const [activePresetId, setActivePresetId] = useState<string | null>(null);
  const [downloadName] = useState("breadcrumb-component");
  ...
  const update = <K extends keyof BreadcrumbState>(key: K, value: BreadcrumbState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_BREADCRUMB_STATE, ...(preset.state as Partial<BreadcrumbState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Breadcrumb Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
